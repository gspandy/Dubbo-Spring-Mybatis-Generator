...
public class WebServerProjectGenerator implements Generator {
    ...
    private List<CgiConfig> cgiConfigList;
    ...
    public WebServerProjectGenerator(String dir, String project, String jsonFile) {
        ...
        this.jsonFileList=jsonFile.split(",");
        this.dir = dir + "-server";
    ...
    public WebServerProjectGenerator(GlobalConfig webConfig) {
        ...
        this.jsonFileList=this.jsonFile.split(",");
        this.dir = getRootDir();
        this.project = webConfig.getName();
        this.projectGenerator = new ProjectGenerator(ConfigGenerator.generateWebProjectConfig(this.globalConfig));
        parseCgiConfig();
    }

    private void parseCgiConfig() {
        this.cgiConfigList = new ArrayList<>();
        for (String jsonFile : this.jsonFileList) {
            final ObjectMapper mapper = new ObjectMapper(new JsonFactory());
            try {
                CgiConfig config = mapper.readValue(new FileInputStream(jsonFile), CgiConfig.class);
                if (config != null) {
                    this.cgiConfigList.add(config);
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void createDtoFromJson() {
        for(String json:this.jsonFileList){
            ...
            dtoConfig.setJsonConfigPath(json);
    ...
    private List<String> getUrls() {
        ...
        for (CgiConfig cgiConfig : this.cgiConfigList) {
            List<String> urls = cgiConfig.getControllerConfigList().stream()
                    .map(ControllerConfig::getCgiInfoList).
                            collect(Collectors.toList()).stream().flatMap(Collection::stream).
                            collect(Collectors.toList()).stream().map(CgiInfo::getCgi)
    ...
    private void createController(String configClass) {
        /*this.projectGenerator.createControllerFile(getRootDir(),
                ...
                configClass);*/
        for(CgiConfig config:this.cgiConfigList){
            config.getControllerConfigList().forEach(item -> {
                ...
                try{
                    new ControllerJavaFileGenerator(cfg, dtoGenerator.getDtoConfig())
                            .generateControllerFile(item, configClass);
                }catch (Exception e){
                    logger.error("create controller file error:{}",cfg);
                }
